...
Verificado en este corte: `ruff check src tests` (limpio en todo lo tocado; el único
error que queda en el repo es en un archivo de insumos que no es de esta sesión),
`mypy src` (701 archivos, sin issues), `lint-imports` (17 contratos, todos OK),
`pytest tests/unit/domain/liquidaciones tests/unit/application/liquidaciones` (26/26).

## Pendiente

1. **Tests de `ImportarLiquidacion`** — todavía no escritos. Van en
   `tests/unit/application/liquidaciones/test_importar_liquidacion.py`, mismo patrón
   `World` que `test_reanalizar_liquidacion.py`. Necesitan un
   `FakeLiquidacionFileParser` (no existe todavía — agregarlo a
   `tests/unit/domain/liquidaciones/fakes.py`) que devuelva un `ResultadoImportacion`
   fijo sin pasar por pandas. Casos mínimos a cubrir:
   - prestador inexistente → `PrestadorNoEncontradoError`.
   - happy path: crea liquidación con `total_incidentes`/`total_importe` derivados del
     parseo, llama `bulk_create` de incidentes, dispara `ReanalizarLiquidacion` y
     devuelve sus totales.
2. **Tests de dominio de las funciones puras de parsing** — sin pandas:
...
3. **Smoke test del endpoint de importación con un archivo real** — el resto de los
...
4. **Frontend** — nada arrancado todavía. Requiere handoff de diseño (`design_handoff_liquidaciones/README.md`
   ya existe en el repo, pero conviene reconfirmarlo/ajustarlo con el usuario antes de
   construir, según convención del proyecto). Pantallas: Dashboard, Liquidaciones,
   Prestadores, SPSTs, Tarifarios, Tabla KM (6 pantallas, ver el handoff).
5. **Catálogo de permisos** — `liquidaciones.is_enabled` sigue en `False`. Activarlo
   solo cuando el usuario lo pida explícitamente, una vez que exista frontend (mismo
   criterio ya aplicado en insumos).
6. **Casos de uso de escritura para el resto de las entidades de configuración**
...
Completar el punto 1 (tests de `ImportarLiquidacion`) es lo más chico y natural para
retomar: ya está el use case, el endpoint y los fakes base — solo falta el
`FakeLiquidacionFileParser` y los tests en sí.
