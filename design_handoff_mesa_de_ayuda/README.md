...
## Screen: Login
- **Layout**: split screen. Panel izquierdo 42% ancho, fondo `#232323`, dos manchas radiales decorativas (naranja arriba-derecha, gris abajo-izquierda) + marca de agua del ícono de marca (abanico, `assets/icon-mark-blanco.png`) al 8% de opacidad, 340px, esquina inferior derecha. Contenido centrado: logo blanco (`assets/logo-blanco.png`, 36px alto), separador naranja 48×2px, título "Plataforma Unificada de Operaciones" (Montserrat 800 25px, blanco), texto "Insumos, liquidaciones, vacaciones, análisis de logs de impresoras y monitoreo, todo en un solo lugar." (14.5px, blanco 65% opacidad).
- Panel derecho (flex:1, fondo `#FAFAF8`): formulario centrado, ancho fijo 360px. Título "Iniciar sesión" (Montserrat 800 25px) + subtítulo gris. Campos "Usuario" y "Contraseña" (label 600 13px `#4b4b4b` + input radio 8px, borde `rgba(0,0,0,.15)`), link "¿Olvidaste tu contraseña?" alineado a la derecha del label de contraseña, checkbox "Recordarme" (accent naranja), botón primario "Ingresar" full-width naranja. Footer: "Portal interno · Canal Directo" (12px, `#b5b5b5`).
- Sin lógica de autenticación real — es solo la interfaz; conectar al proveedor de auth real del proyecto.
...
- `Login Mesa de Ayuda.dc.html` — pantalla de login, ver sección "Screen: Login".
- `assets/` — todos los PNG referenciados arriba, incluye `icon-mark-blanco.png` (ícono de marca recortado, usado como watermark en el login).
