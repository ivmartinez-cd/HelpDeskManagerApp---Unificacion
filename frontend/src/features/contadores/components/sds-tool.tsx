...
import { contadoresApi, type SdsClient } from "../api/contadores-api";
import { ProcessClientModal } from "./process-client-modal";

export function SdsTool() {
  const [clients, setClients] = useState<SdsClient[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");

  const [processOpen, setProcessOpen] = useState(false);
  const [processingClient, setProcessingClient] = useState<SdsClient | null>(null);
  ...
  useEffect(() => {
    ...
    contadoresApi
      ...
      .catch((err: unknown) => {
        if (active) {
          const message = err instanceof Error ? err.message : "Error al cargar clientes HP SDS";
          toast.error(message);
          setLoading(false);
        }
      });
  ...
  const handleToggleSumaColor = async (client: SdsClient, newValue: boolean) => {
    try {
      await contadoresApi.updateSdsConfig(client.id, {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Buscar por cliente SDS..."
      ...
      {loading ? (
        ...
        <div className="flex h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card p-6 text-center">
          <Printer className="h-10 w-10 text-muted-foreground/40 mb-2" />
          <p className="text-sm font-medium text-muted-foreground">
            No se encontraron clientes HP SDS
          </p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border bg-muted/40 uppercase tracking-wider text-muted-foreground font-bold">
                <tr>
                  <th className="px-4 py-3">Cliente HP SDS</th>
