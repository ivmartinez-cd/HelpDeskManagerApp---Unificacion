...
/** Leyenda "te toca estar en..." vinculada al login: cruza el usuario
 * logueado contra los turnos de HOY que ya trae `useTurnosHoy()` (sin fetch
 * propio). Si el usuario no tiene ningún turno hoy, no se renderiza nada --
 * es un aviso personal, no un resumen del equipo (para eso ya está la card
 * "Turnos del día"). */
export function MiTurnoBanner({
  ...
  return (
    <div className="flex flex-none flex-wrap items-center gap-2 rounded-[12px] border border-brand-orange/30 bg-brand-orange/[0.08] px-4 py-2.5">
      <CalendarClock className="h-4 w-4 shrink-0 text-brand-orange" />
      <span className="font-body text-[13px] font-semibold text-foreground">Hoy te toca:</span>
      <div className="flex flex-wrap items-center gap-2">
        {propios.map((shift) => {
          const activo = shift.horaInicio <= nowStr && nowStr < shift.horaFin;
          return (
            <span
              key={shift.slotId}
              className={
