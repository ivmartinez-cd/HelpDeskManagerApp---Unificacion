 ...
 * ssr:false — ver preventivos-mapa-icono.ts. Acá la librería se carga con
 * `import()` dentro de un efecto, nunca a nivel de módulo. */
export function PreventivosMapaCanvas({ puntos }: { puntos: PuntoMapaPreventivo[] }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      ...
      markersRef.current = puntos
        .filter((p) => p.ubicado && p.latitud !== null && p.longitud !== null)
        .map((punto) => {
      ...
      const posiciones: [number, number][] = markersRef.current.map((m) => {
