...
class SolicitudTvModel(Base):
    ...
    El nombre de tabla es heredado: `SolicitudTv` vivió en el módulo
    `bono_tecnicos` hasta que se separó a su propio módulo `tareas_varias`
    (las TV aprobadas siguen sumando al Puntaje, pero por lectura vía
    `TareasVariasGateway`, no porque compartan módulo). No se renombró la
    tabla para no migrar datos reales de dev sin necesidad."""
