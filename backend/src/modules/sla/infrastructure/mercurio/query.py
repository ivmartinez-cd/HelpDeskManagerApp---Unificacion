"""La consulta legacy de cumplimiento de SLA contra Siges (ver SLA.md) —
parametrizada con placeholders pyodbc, no se interpola nada (ARCHITECTURE_GUIDE
§8). Reindentada respecto del original solo para el largo de línea; no
modificar los CASE calculados: la paridad con la planilla de Excel que este
módulo reemplaza depende de ellos."""
...
IT.Tiempo,
CASE WHEN CAST(SUBSTRING(IT.Tiempo, 1, CHARINDEX(':', IT.Tiempo) - 1) AS INT)
        BETWEEN 0 AND 23 THEN '0 a 24'
    WHEN CAST(SUBSTRING(IT.Tiempo, 1, CHARINDEX(':', IT.Tiempo) - 1) AS INT)
        BETWEEN 24 AND 47 THEN '24 a 48'
    WHEN CAST(SUBSTRING(IT.Tiempo, 1, CHARINDEX(':', IT.Tiempo) - 1) AS INT)
        BETWEEN 48 AND 71 THEN '48 a 72'
    WHEN CAST(SUBSTRING(IT.Tiempo, 1, CHARINDEX(':', IT.Tiempo) - 1) AS INT)
        BETWEEN 72 AND 95 THEN '72 a 96'
    ELSE '96hs o mas' END AS RANGO,
I.SLA,
CASE WHEN CAST(SUBSTRING(IT.Tiempo, 1, CHARINDEX(':', IT.Tiempo) - 1) AS INT) <= I.SLA
        THEN '0'
    ELSE CAST((CAST(SUBSTRING(IT.Tiempo, 1, CHARINDEX(':', IT.Tiempo) - 1) AS INT) - I.SLA)
        AS VARCHAR) END AS HorasVencido,
CASE WHEN CAST(SUBSTRING(IT.Tiempo, 1, CHARINDEX(':', IT.Tiempo) - 1) AS INT) < I.SLA
    THEN 'Correcto' ELSE 'Vencido' END AS Resultado,
IT.Reloj,
