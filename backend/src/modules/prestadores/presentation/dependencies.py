...
de host y el runner compartido vienen de `require_orion_runner` (ADR-018)."""
...
def get_prestador_siges_gateway() -> PyodbcPrestadorGateway:
    return PyodbcPrestadorGateway(require_orion_runner())
...
def get_prestador_siges_gateway_or_none() -> PyodbcPrestadorGateway | None:
    ...
    usando la variante estricta porque sin ORION no tiene nada que hacer."""
    try:
        return get_prestador_siges_gateway()
    except ExternalServiceError as exc:
        logger.warning("Siges (ORION) no configurado; se sigue sin conteo en vivo", exc_info=exc)
        return None
