...
## Otro pendiente de esta sesión (no relacionado a datos)

`CLAUDE.md` todavía documenta el comportamiento de hot-reload stale de Turbopack en
Docker (sección "Frontend en Docker: Turbopack no siempre recompila") — quedó
desactualizado el 2026-08-13 al sacar `next dev`/Turbopack del contenedor frontend
(pasó a `next build && next start`, ver memoria de sesión
`project_frontend_dev_container_stale_reload`). Falta decidir si se reescribe esa
sección o se borra.
