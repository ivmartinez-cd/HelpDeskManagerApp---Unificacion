...
def _use_case(
    ...
    return GetPendingClientsUseCase(GetCalendarEventsUseCase(repo, overrides))
...
async def test_pendiente_de_un_dia_aparece() -> None:
    ...
    # La fecha del evento es hoy-1; el backend no calcula días de atraso,
    # solo devuelve los eventos — el cálculo de "hace N días" lo hace el front.
...
async def test_ordena_del_mas_viejo_al_mas_nuevo() -> None:
    ...
    # El repo devuelve en orden distinto al esperado a propósito.
...
@pytest.mark.asyncio
async def test_evento_cubierto_por_override_viaja_anotado() -> None:
    """Un pendiente cubierto por override debe viajar con cobertura != None —
    misma lógica que GetCalendarEventsUseCase para los eventos de hoy."""
    ayer = _TODAY - timedelta(days=1)
    e = _event("cubierto", ayer)
    repo = _mock_repo([e])
...
async def test_superadmin_ve_pendientes_de_todos() -> None:
    ...
    repo = _mock_repo()
    # Superadmin: list_events se llama sin filtro de operador (operador_id=None).
