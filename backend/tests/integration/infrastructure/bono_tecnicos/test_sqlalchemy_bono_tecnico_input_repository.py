...
async def test_upsert_inserta_y_luego_actualiza_la_misma_fila(db_session: AsyncSession) -> None:
    ...
    await repo.upsert(
        BonoTecnicoInput(id_tecnico=id_tecnico, periodo=202605, tecnico="CD - Ana", dias=17)
    )
    ...
    await repo.upsert(
        BonoTecnicoInput(id_tecnico=id_tecnico, periodo=202605, tecnico="CD - Ana", dias=20)
    )
...
async def test_find_by_periodo_no_trae_otros_periodos(db_session: AsyncSession) -> None:
    ...
    await repo.upsert(
        BonoTecnicoInput(id_tecnico=id_tecnico, periodo=202606, tecnico="CD - Ana", dias=18)
    )
